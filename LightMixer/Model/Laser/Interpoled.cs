...
namespace LaserDisplay
{
    ...
    public class ResamplingService
    {
        ...
        public short[] Resample(short[][,] input)
        {
            ...
            {
                arr[x] = new uint[s.GetLength(0), s.GetLength(1)];
                for (x1 = 0; x1 < s.GetLength(0); x1++)
                {
                    for (y1 = 0; y1 < s.GetLength(1); y1++)
                    {
                        arr[x][x1, y1] = FituInt(s[x1, y1] + 32767);
                    }
                }
                x++;
            }
            ...
            x = 0;
            short[] arrres = new short[res[0].GetLength(1)];
            for (y1 = 0; y1 < res[0].GetLength(1); y1++)
            {
                arrres[y1] = FitShort((int)res[0][0, y1] - 32767);
            }
        ...
        private uint FituInt(int x)
        {
            ...
            else return Convert.ToUInt32(x);
        }

        private short FitShort(int r)
        {
        ...
        public uint[][,] Resample(uint[][,] input)
        {
            ...
            // create bitmaps
            uint[][,] work = new uint[planes][,];
            uint[][,] output = new uint[planes][,];
            int c = 0;

            for (c = 0; c < planes; c++)
            {
                work[c] = new uint[nWidth, height];
                output[c] = new uint[nWidth, nHeight];
            }
            ...
            for (c = 0; c < planes; c++)
            {
                for (k = 0; k < height; k++)
                {
                    for (i = 0; i < nWidth; i++)
                    {
                        ...
                        work[c][i, k] = (uint)Math.Min(Math.Max(intensity / contrib[i].wsum, UInt16.MinValue), UInt16.MaxValue);
                    }
            ...
            for (c = 0; c < planes; c++)
            {
                for (k = 0; k < nWidth; k++)
                {
                    for (i = 0; i < nHeight; i++)
                    {
                        ...
                        output[c][k, i] = (uint)Math.Min(Math.Max(intensity, UInt16.MinValue), UInt16.MaxValue);
                    }
    ...
    #endregion ResamplingFilter class
}