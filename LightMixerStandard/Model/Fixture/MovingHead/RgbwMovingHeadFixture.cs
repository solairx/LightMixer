...
namespace LightMixer.Model.Fixture
{
    public class RgbwMovingHeadFixture : MovingHeadFixture
    {
        ...
        public override byte?[] Render()
        {
            ...
            else if (this.currentEffect?.GetType() == typeof(MovingHeadFlashAll) && RedValue + GreenValue + BlueValue > 100 && LightMixerBootStrap.UnityContainer.Resolve<DmxChaser>().CurrentLedEffect?.GetType() != typeof(ZoneRotateEffect))
            {
                byte?[] arr = new byte?[512];
                arr[StartDmxAddress] = 22;
                arr[StartDmxAddress + 1] = 0;
